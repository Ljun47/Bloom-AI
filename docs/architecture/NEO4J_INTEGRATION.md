...
9. [Neo4j vs RDB 데이터 괴리 — 설계 결정](#9-neo4j-vs-rdb-데이터-괴리--설계-결정-2026-04-09)
10. [차후 계획 — 조직 단위 분석 확장](#10-차후-계획--조직-단위-분석-확장)
11. [설정·환경변수·테스트](#11-설정환경변수테스트)
12. [부록: STORAGE_MODE 참고](#12-부록-storage_mode-참고)
...
└──────────────────────┬──────────────────────────────────┘
    ...
    └──────────────────────────────────────┘
                ...
                ▼             ▼
          ┌──────────┐ ┌──────────────────────────────┐
          │ ① Neo4j  │ │ ② RDB 누적 저장 (Mode A)      │
          │ 그래프   │ │ AI서버가 EMA 계산 후 PUT       │
          │ 직접 저장│ │ GET /graph_nodes (기존 조회)   │
          │          │ │ EMA 병합                       │
          └──────────┘ │ PUT /graph_nodes (저장)        │
                       └──────────────────────────────┘
           GoTNode를    프론트엔드 시각화 서빙용 누적 데이터
           노드/엣지로   (label+grp 기준 UPSERT)
           MERGE
```

**① Neo4j 직접 저장** — GoT 노드와 연결 관계를 그래프 DB에 저장한다.
세션별 GoT 원본 이력 보존용. 이후 패턴 분석 등에 활용 가능하다.

**② RDB 누적 저장 (Mode A)** — AI서버가 직접 EMA를 계산하여 Backend의 MySQL에 저장한다.
`publish_graph_to_rdb()` → GET /graph_nodes(기존 누적 조회) → EMA 병합 → PUT /graph_nodes.
프론트엔드는 이 데이터를 Backend를 통해 직접 조회한다.

> **중요**: ①은 Neo4j가 없으면 실패하지만, **파이프라인은 계속 진행**된다.
> ②는 Neo4j와 무관하게 항상 동작한다.
...
프론트엔드 (app-4:3000)
    │
    │  "사용자 A의 감정 패턴 보여줘"
    │  GET /api/graph/users/{user_id}/data
    ▼
...
└── podcast_reasoning.py         ← _save_got_to_neo4j() + publish_graph_to_rdb() 호출
...
### 6-3. RDB 누적 저장 흐름 (Mode A — 현행)

AI서버가 EMA 계산까지 직접 수행하고, Backend에 PUT으로 저장한다.
...
│                                                  │
│ 1. GET /api/v1/graph_nodes?user_id=X             │
│    → 기존 누적 데이터 조회                         │──GET──▶ Backend (app-3)
│    → 신규 사용자 시 빈 GraphCumulativeData 반환   │◀──────
│                                                  │
│ 2. EMA 병합 (AI서버에서 직접 계산)                │
│    new_weight = 0.3 × intensity + 0.7 × old_weight │
│    label+grp 기준 UPSERT                          │
│                                                  │
│ 3. PUT /api/v1/graph_nodes                       │──PUT──▶ Backend (app-3)
│    { user_id,                                    │
│      type="graph_cumulative",                    │
│      data: {nodes, links} }                      │
│    ※ session_id/timestamp 미포함 (BE 거부)        │
└──────────────────────────────────────────────────┘
```

AI서버 참조 코드: `src/api/graph_cumulative.py` — `publish_graph_to_rdb()`

**BE 호환성 테스트 완료 (2026-04-09)**:
- GET (신규유저): ✅ 200 + 빈 nodes/links
- GET (기존유저): ✅ 200 + `data.data` 이중 중첩 구조
- PUT: ✅ 200 (`session_id`/`timestamp` 제거 후 확인)
- 신규유저 GET→PUT→GET 전체 플로우: ✅ PASS
...
**엔드포인트**: `POST /api/graph_nodes/episodes`
...
```
POST /api/graph_nodes/episodes 수신
    │
...
| 순서 | 엔드포인트 | 메서드 | 용도 |
|------|-----------|--------|------|
| **①** | `/api/graph_nodes/episodes` | POST | AI서버 → GoT 에피소드 원본 수신 + EMA 계산 + UPSERT |
...
| 2 | `GET /api/graph/users/{id}/data` | **프론트엔드용 — MySQL에서 직접 서빙** |
...
| 3-B-2 | GoT 에피소드 수신 + EMA 계산 + UPSERT API | `POST /api/graph_nodes/episodes` |
| 3-B-3 | 프론트엔드용 누적 그래프 조회 API | `GET /api/graph/users/{id}/data` |
...
## 9. Neo4j vs RDB 데이터 괴리 — 설계 결정 (2026-04-09)

### 9-1. 현재 구조에서 발생하는 괴리

동일한 GoT 결과가 메모리에서 두 저장소로 **독립적으로** 분기된다:

```
GoT 결과 (메모리)
  ├─→ Neo4j: 에피소드별 GoTNode로 개별 저장 (ID: {episode_id}_{node_id})
  └─→ RDB:   EMA 가중치로 병합된 누적 단일 그래프 저장
```

결과적으로 두 저장소의 데이터 형태가 달라진다:

| | Neo4j | RDB (/graph_nodes) |
|--|-------|-------------------|
| 저장 단위 | 세션별 GoT 그래프 (독립) | 사용자별 단일 누적 그래프 |
| 노드 식별 | `{episode_id}_{node_id}` — 에피소드마다 별개 | `label+grp` — 동일 개념은 하나의 행 |
| 가중치 | GoT 원본 intensity 그대로 | EMA 평활화 누적값 |
| 사용자 지속 사용 시 | 세션 수만큼 노드 증가 | 노드는 label+grp 기준 UPSERT |

### 9-2. 검토한 옵션

| 옵션 | 설명 | 장점 | 단점 |
|------|------|------|------|
| **A: 현행 유지** | 두 저장소가 다른 목적으로 독립 운용 | 변경 없음, 목적 분리 명확 | 일관성 보장 불가 |
| **B: Neo4j SSOT** | Neo4j를 단일 진실 소스로, RDB는 Neo4j에서 재계산 | RDB 재현 가능 | Neo4j 조회 로직 추가 필요 |
| **C: RDB 단일화** | Neo4j 제거, RDB에 세션별 원본 + 누적 테이블 분리 | 저장소 단순화 | Neo4j 그래프 탐색 이점 포기 |

### 9-3. 결정 사항

**Option A (현행 유지)** — 두 저장소의 목적이 다르므로 괴리를 설계상 허용한다.

- **Neo4j**: 세션별 GoT 원본 이력 보존. 시계열 패턴 분석, 감정 흐름 추적용.
- **RDB**: EMA 누적 단일 그래프. 프론트엔드 실시간 시각화 서빙용.

### 9-4. 향후 전환 조건 (Option B 전환 기준)

아래 요구가 발생하면 **Option B(Neo4j SSOT)**로 전환을 검토한다:

- "특정 시점 기준으로 RDB 누적 데이터를 재계산해야 한다"
- "Neo4j와 RDB의 값이 다른 것이 운영 문제가 된다"
- "RDB가 유실됐을 때 Neo4j에서 완전 복구가 필요하다"

전환 시 필요한 작업:
1. Neo4j GoTNode에 타임스탬프 저장 (현재 `episode_id`만 있음)
2. 사용자별 세션 순서대로 GoTNode를 조회하는 Cypher 쿼리 구현
3. 조회 결과를 시간순으로 EMA 재계산하는 로직 구현
4. RDB를 캐시로 전환 (SSOT는 Neo4j)

---

## 10. 차후 계획 — 조직 단위 분석 확장
...
| EMA 누적 그래프 → RDB | `src/api/graph_cumulative.py` | `publish_graph_to_rdb()` |
| 누적 그래프 GET/PUT | `src/api/client.py` | `load_graph_cumulative()` / `put_graph_cumulative()` |
