...
main ← PR 머지 (3명 전원 승인 필수)
 └── develop ← 통합 테스트 브랜치 (최소 1명 리뷰)
      ├── feature/analysis-*      (Dev-A: 분석 도메인)
      ├── feature/reasoning-*     (Dev-B: 추론/생성 도메인)
      └── feature/validation-*    (Dev-C: 검증/부가 도메인)
```
...
- `feature/analysis-*`: Dev-A 전용. 분석 에이전트 개발.
- `feature/reasoning-*`: Dev-B 전용. 추론/생성 에이전트 개발.
- `feature/validation-*`: Dev-C 전용. 검증/부가 에이전트 개발.
- `fix/*`: 버그 수정. 도메인 접두사 포함 권장 (예: `fix/analysis-emotion-null-check`)
- `hotfix/*`: 긴급 수정. main에서 분기 후 직접 main에 머지.

### 브랜치 네이밍 예시

```
feature/analysis-intent-classifier    # Dev-A: Intent Classifier 구현
feature/analysis-emotion-agent        # Dev-A: Emotion Agent 구현
feature/reasoning-synthesis           # Dev-B: Synthesis Agent 구현
feature/reasoning-memory-rag          # Dev-B: Memory Agent RAG 구현
feature/validation-safety-crisis      # Dev-C: Safety Agent 위기 대응
feature/validation-visualization      # Dev-C: Visualization Agent 구현
```
